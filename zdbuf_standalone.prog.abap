...
CLASS zcx_dbuf_auth_error DEFINITION DEFERRED.
CLASS zcx_dbuf_error DEFINITION DEFERRED.
CLASS zcx_dbuf_file_error DEFINITION DEFERRED.
CLASS zcx_dbuf_mapping_error DEFINITION DEFERRED.
CLASS zcx_dbuf_validation_error DEFINITION DEFERRED.
...
CLASS zcl_dbuf_huge_xlsx_reader DEFINITION
  PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
    INTERFACES zif_dbuf_file_reader.
ENDCLASS.
  ...
  METHOD zif_dbuf_file_reader~read.
    ...
    TRY.
      ...
      CATCH cx_root INTO DATA(exc).
        RAISE EXCEPTION NEW zcx_dbuf_file_error( text     = |Huge XLSX read failed: { exc->if_message~get_text( ) }|
                                                 previous = exc ).
    ENDTRY.
  ...
  METHOD zif_dbuf_db_committer~commit.
    ...
    TRY.
        ...
        ELSE.
          ...
          RAISE EXCEPTION NEW zcx_dbuf_validation_error( text = result-message ).
        ENDIF.
...
CLASS zcl_dbuf_result_xlsx_writer IMPLEMENTATION.
  METHOD zif_dbuf_result_writer~write.
    ...
    ws->set_cell( ip_column = 'A'
                  ip_row    = 1
                  ip_value  = 'Row#' ).
    ws->set_cell( ip_column = 'B'
                  ip_row    = 1
                  ip_value  = 'Status' ).
    ws->set_cell( ip_column = 'C'
                  ip_row    = 1
                  ip_value  = 'Message' ).
    ws->set_cell( ip_column = 'D'
                  ip_row    = 1
                  ip_value  = 'Raw Data' ).
    ...
    TRY.
      ...
      CATCH cx_root INTO DATA(exc).
        RAISE EXCEPTION NEW zcx_dbuf_file_error( text     = |XLSX write failed: { exc->if_message~get_text( ) }|
                                                 previous = exc ).
    ENDTRY.
  ENDMETHOD.
ENDCLASS.
...
CLASS zcl_dbuf_xlsm_reader IMPLEMENTATION.
  METHOD zif_dbuf_file_reader~read.
    ...
    TRY.
        reader->load_data( EXPORTING iv_data = file_content CHANGING io_excel = excel ).
      CATCH cx_root INTO DATA(exc).
        RAISE EXCEPTION NEW zcx_dbuf_file_error( text     = |XLSM read failed: { exc->if_message~get_text( ) }|
                                                 previous = exc ).
    ENDTRY.
    DATA(iterator) = excel->get_worksheets_iterator( ).
    WHILE iterator->has_next( ) = abap_true.
...
CLASS zcl_dbuf_xlsx_reader DEFINITION
  PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
    INTERFACES zif_dbuf_file_reader.
ENDCLASS.
