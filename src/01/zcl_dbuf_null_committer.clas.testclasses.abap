...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD commit_never_writes_db.
    ...
    ASSIGN tr->* TO <t>.
    APPEND 'dummy' TO <t>.
    DATA(result) = cut->zif_dbuf_db_committer~commit( table_name = 'ZTESTXXX'
                                                      table_ref  = tr ).
    cl_abap_unit_assert=>assert_equals( act = result-rows_committed
                                        exp = 0 ).
  ENDMETHOD.
  ...
  METHOD commit_returns_zero_rows.
    ...
    DATA(result) = cut->zif_dbuf_db_committer~commit( table_name = 'ZTESTXXX'
                                                      table_ref  = tr ).
    cl_abap_unit_assert=>assert_equals( act = result-rows_failed
                                        exp = 0 ).
  ENDMETHOD.
  ...
  METHOD empty_table_still_works.
    CREATE DATA DATA(tr) TYPE TABLE OF string.
    TRY.
        cut->zif_dbuf_db_committer~commit( table_name = 'ZTESTXXX'
                                           table_ref  = tr ).
