...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD valid_row_returns_valid.
    ...
    DATA(result) = cut->validate_row(
      row_index = 1 cells = VALUE string_table( ( 'ABAP' ) )
      mappings = mappings table_name = 'ZTESTXXX' ).
    cl_abap_unit_assert=>assert_equals( act = result-is_valid exp = abap_true ).
  ENDMETHOD.
  ...
  METHOD missing_cell_returns_invalid.
    ...
    DATA(result) = cut->validate_row(
      row_index = 5 cells = VALUE string_table( ( 'ONLY_ONE' ) )
      mappings = mappings table_name = 'ZTESTXXX' ).
    cl_abap_unit_assert=>assert_equals( act = result-is_valid exp = abap_false ).
    cl_abap_unit_assert=>assert_char_cp( act = result-message exp = '*Row 5*' ).
  ENDMETHOD.

  METHOD never_raises_exception.
    TRY.
        cut->validate_row(
          row_index = 99 cells = VALUE string_table( )
  ...
  METHOD empty_mappings_returns_valid.
    DATA(result) = cut->validate_row(
      row_index = 1 cells = VALUE string_table( ( 'A' ) )
      mappings = VALUE zcl_dbuf_column_mapper=>column_mappings( ) table_name = 'ZTESTXXX' ).
    cl_abap_unit_assert=>assert_equals( act = result-is_valid exp = abap_true ).
  ENDMETHOD.
