...
CLASS ltcl_test IMPLEMENTATION.
  ...
  METHOD csv_single_row.
    ...
    cl_abap_unit_assert=>assert_equals( act = lines( sheets[ 1 ]-rows ) exp = 1 ).
    cl_abap_unit_assert=>assert_equals( act = sheets[ 1 ]-rows[ 1 ]-cells[ 1 ] exp = 'A' ).
    cl_abap_unit_assert=>assert_equals( act = sheets[ 1 ]-rows[ 1 ]-cells[ 3 ] exp = 'C' ).
  ENDMETHOD.
  ...
  METHOD tsv_separator.
    ...
    DATA(sheets) = cut->zif_dbuf_file_reader~read( xdata ).
    cl_abap_unit_assert=>assert_equals( act = lines( sheets[ 1 ]-rows[ 1 ]-cells ) exp = 3 ).
  ENDMETHOD.

  METHOD quoted_comma_not_split.
    DATA(xdata) = cl_abap_codepage=>convert_to( '"Hello,World",B' ).
    DATA(sheets) = cut->zif_dbuf_file_reader~read( xdata ).
    cl_abap_unit_assert=>assert_equals( act = lines( sheets[ 1 ]-rows[ 1 ]-cells ) exp = 2 ).
  ENDMETHOD.
  ...
  METHOD empty_lines_skipped.
    ...
    DATA(sheets) = cut->zif_dbuf_file_reader~read( xdata ).
    cl_abap_unit_assert=>assert_equals( act = lines( sheets[ 1 ]-rows ) exp = 2 ).
  ENDMETHOD.
